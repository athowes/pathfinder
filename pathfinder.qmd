...
* Why not mixture of Gaussians? Yes it's in the paper but not all the way there yet. Seems related to INLA
* Pathfinder is like early-stopping VI but with 1) automated stopping 2) additional entropy and mode avoidance
* Uses covariances from L-BFGS row-rank plus diagonal inverse Hessian approximation
* KL-divergence evaluated using Monte Carlo
* Stan uses 75 HMC iterations for phase I warmup
  * Sufficient for most problems but can be very expensive
...
* Wanted to avoid using KL-divergence (as this is in the design of the algorithm)
  * Use Wasserstein-1 distance
...
* Partial solution run Pathfinder >1 times (multi-Pathfinder)
  ...
  * Importance resample the draws with Pareto smoothing (PSIS I imagine)
